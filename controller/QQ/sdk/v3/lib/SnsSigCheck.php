...
class SnsSigCheck
{
    ...
    static public function makeBackSig($method, $url_path, $params, $secret)
    {
    	$strs = strtoupper($method) . '&' . rawurlencode($url_path) . '&';
    	...
    	foreach ($params as $key => $val )
    	{
    		$val = preg_replace("/[^0-9a-zA-Z!\\*\\(\\)]/e", "'%'.strtoupper(dechex(ord('$0')))", $val);
    		array_push($query_string, $key . '=' . $val);
    	}

    	$query_string = join('&', $query_string);
    	
    	$mk = $strs . rawurlencode($query_string);
    	$my_sign = hash_hmac("sha1", $mk, strtr($secret, '-_', '+/'), true);
    	$my_sign = base64_encode($my_sign);
     ...
     * ASCII码的十六进制加%
     */
    static private function getStr($matches)
    {
    	$str = '%'.dechex(ord($matches[0]));
    	return $str;
    }
}
// end of script